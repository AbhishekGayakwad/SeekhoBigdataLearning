...
object Question10 {
  ...
  def validateHeaderTrailer(df: DataFrame, fileType: String, metadataDF: DataFrame): (Boolean, Dataset[Row]) = {
    ...
    df.show()
    val Df1 = df.filter(col("name") =!= "CUSTOMER_INFO" && col("name") =!= "TRAILER").select(col("name"),col("address"))
    Df1.show()
  ...
  def main(args: Array[String]): Unit = {
    val filePath = "C:\\Users\\Admin\\Desktop\\Seekho BigData\\Data\\Question10.txt" // Path to your file (adjust path)
    val fileType = "CUSTOMER_INFO" // Specify file type for metadata lookup
    ...
    try {
      ...
      val metadataData = Seq(
        Row("CUSTOMER_INFO", "v1", 5) // Updated expected record count to 22
      )
      ...
      if (isValid._1) {
        ...
        val result = isValid._2.filter(col("address").isNotNull)
          result.show()
        }
      else{
