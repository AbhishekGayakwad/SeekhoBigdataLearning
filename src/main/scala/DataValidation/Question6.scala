...
object Question6 {
  ...
  def validateHeaderTrailer(df: DataFrame, fileType: String, metadataDF: DataFrame): (Boolean, Dataset[Row]) = {
    ...
    df.show()
    val Df1 = df.filter(col("Name") =!= "CUSTOMER" && col("Name") =!= "TRAILER").select(col("NAME"),col("age"))
    Df1.show()
  ...
  def main(args: Array[String]): Unit = {
    val filePath = "C:\\Users\\Admin\\Desktop\\Seekho BigData\\Data\\Question6.txt" // Path to your file (adjust path)
    val fileType = "CUSTOMER" // Specify file type for metadata lookup
    ...
    try {
      ...
      val metadataData = Seq(
        Row("CUSTOMER", "v3", 6) // Updated expected record count to 22
      )
      ...
      if (isValid._1) {
          val result = isValid._2.select(col("name"),when(col("age")>=20 && col("age")<=30,"20-30")
            .when(col("age")>=31 && col("age")<=40,"31-40")
            .when(col("age")>=41 && col("age")<=50,"41-50")
            .when(col("age")>=51 && col("age")<=60,"51-60").alias("group"))
          result.show()
        }
      else{
