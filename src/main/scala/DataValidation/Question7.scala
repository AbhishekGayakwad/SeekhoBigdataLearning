...
object Question7 {
  ...
  def validateHeaderTrailer(df: DataFrame, fileType: String, metadataDF: DataFrame): (Boolean, Dataset[Row]) = {
    ...
    df.show()
    val Df1 = df.filter(col("Name") =!= "TIMESHEET" && col("Name") =!= "TRAILER").select(col("NAME"),col("hours"))
    Df1.show()
  ...
  def main(args: Array[String]): Unit = {
    val filePath = "C:\\Users\\Admin\\Desktop\\Seekho BigData\\Data\\Question7.txt" // Path to your file (adjust path)
    val fileType = "TIMESHEET" // Specify file type for metadata lookup
    ...
    try {
      ...
      val metadataData = Seq(
        Row("TIMESHEET", "v1", 6) // Updated expected record count to 22
      )
      ...
      if (isValid._1) {
          val result = isValid._2.groupBy(col("name")).agg(sum(col("hours")))
          result.show()
        }
      else{
