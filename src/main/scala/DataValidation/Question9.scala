...
object Question9 {
  ...
  def loadFileByExtension(filePath: String): DataFrame = {
    ...
    val dfDataSchema = StructType(List(
      StructField("salesPerson", StringType, nullable = false),
      StructField("sales", StringType, nullable = false),
      StructField("extra", StringType, nullable = false)
    ))
  ...
  def validateHeaderTrailer(df: DataFrame, fileType: String, metadataDF: DataFrame): (Boolean, Dataset[Row]) = {
    ...
    val actualRecordCount = df.count() - 2 // Exclude header and trailer rows from the count q
    df.show()
    val Df1 = df.filter(col("salesPerson") =!= "SALES" && col("salesPerson") =!= "TRAILER").select(col("salesPerson"),col("sales").cast("Int"))
    Df1.show()
  ...
  def main(args: Array[String]): Unit = {
    val filePath = "C:\\Users\\Admin\\Desktop\\Seekho BigData\\Data\\Question9.txt" // Path to your file (adjust path)
    val fileType = "SALES" // Specify file type for metadata lookup
    ...
    try {
      ...
      val metadataData = Seq(
        Row("SALES", "v2", 7) // Updated expected record count to 22
      )
      ...
      if (isValid._1) {
        val result = isValid._2.groupBy(col("salesPerson")).agg(max(col("sales")),min(col("sales")))
          result.show()
        }
      else{
