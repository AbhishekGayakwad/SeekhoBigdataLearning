...
object Question3 {
  ...
  def validateHeaderTrailer(df: DataFrame, fileType: String, metadataDF: DataFrame): (Boolean, Dataset[Row]) = {
    ...
    df.show()
    val Df1 = df.filter(col("Region") =!= "SALES" && col("Region") =!= "TRAILER")
    Df1.show()
  ...
  def main(args: Array[String]): Unit = {
    val filePath = "C:\\Users\\Admin\\Desktop\\Seekho BigData\\Data\\Question3.txt" // Path to your file (adjust path)
    ...
    try {
      ...
      val metadataData = Seq(
        Row("SALES", "v2", 5) // Updated expected record count to 22
      )
      ...
      if (isValid._1) {
        val result = isValid._2.groupBy(col("region")).agg(sum(col("sales")))
        result.show()
      }
      else{
