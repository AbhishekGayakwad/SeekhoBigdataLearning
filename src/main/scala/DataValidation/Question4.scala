...
object Question4 {
  ...
  def main(args: Array[String]): Unit = {
    val filePath = "C:\\Users\\Admin\\Desktop\\Seekho BigData\\Data\\Question4.txt" // Path to your file (adjust path)
    ...
    try {
      ...
      val metadataData = Seq(
        Row("EMPLOYEE", "v1", 5) // Updated expected record count to 22
      )
      ...
      if (isValid._1) {
          val result = isValid._2.filter(col("department")==="IT" and col("salary")>=75000)
          result.show()
        }
      else{
