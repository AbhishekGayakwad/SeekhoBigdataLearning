...
object Question5 {
  ...
  def validateHeaderTrailer(df: DataFrame, fileType: String, metadataDF: DataFrame): (Boolean, Dataset[Row]) = {
    ...
    df.show()
    val Df1 = df.filter(col("Product") =!= "PRODUCT_SALES" && col("Product") =!= "TRAILER").select(col("Product"),col("Sales"))
    Df1.show()
  ...
  def main(args: Array[String]): Unit = {
    val filePath = "C:\\Users\\Admin\\Desktop\\Seekho BigData\\Data\\Question5.txt" // Path to your file (adjust path)
    val fileType = "PRODUCT_SALES" // Specify file type for metadata lookup
    ...
    try {
      ...
      val metadataData = Seq(
        Row("PRODUCT_SALES", "v1", 5) // Updated expected record count to 22
      )
      ...
      if (isValid._1) {
          val result = isValid._2.filter(col("Sales")===0)
          result.show()
        }
      else{
