...
object Question8 {
  ...
  def validateHeaderTrailer(df: DataFrame, fileType: String, metadataDF: DataFrame): (Boolean, Dataset[Row]) = {
    ...
    df.show()
    val Df1 = df.filter(col("Name") =!= "STUDENTS" && col("Name") =!= "TRAILER").select(col("NAME"),col("score"))
    Df1.show()
  ...
  def main(args: Array[String]): Unit = {
    val filePath = "C:\\Users\\Admin\\Desktop\\Seekho BigData\\Data\\Question8.txt" // Path to your file (adjust path)
    val fileType = "STUDENTS" // Specify file type for metadata lookup
    ...
    try {
      ...
      val metadataData = Seq(
        Row("STUDENTS", "v1", 5) // Updated expected record count to 22
      )
      ...
      if (isValid._1) {
          val result = isValid._2.filter(col("score")>80)
          result.show()
        }
      else{
