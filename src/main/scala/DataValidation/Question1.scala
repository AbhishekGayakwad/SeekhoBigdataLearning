...
import org.apache.spark.sql.functions.{avg, col, trim}
...
object Question1 {
  ...
  def main(args: Array[String]): Unit = {
    val filePath = "C:\\Users\\Admin\\Desktop\\Seekho BigData\\Data\\Question1.txt" // Path to your file (adjust path)
    ...
    try {
      ...
      val metadataData = Seq(
        Row("EMPLOYEE", "v1", 6) // Updated expected record count to 22
      )
      ...
      if (isValid._1) {
          val result = isValid._2.groupBy(col("department")).agg(avg(col("salary")))
          result.show()
        }
      else{
